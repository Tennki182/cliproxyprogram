...
export async function authRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  const pendingCodes = new Map<string, { deviceCode: string; interval: number }>();

  /**
   * POST /auth/device - Start device authorization flow
   */
  fastify.post<{ Body: { interval?: number; timeout?: number } }>(
    '/auth/device',
    {
      schema: {
        body: {
   ...
   */
  fastify.post<{ Body: { device_code: string } }>(
    '/auth/poll',
    {
      schema: {
        body: {
