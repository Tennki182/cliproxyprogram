...
export function saveCredential(credential) {
    ...
    const existing = db.exec(`SELECT id FROM credentials WHERE account_id = ?`, [credential.account_id]);
    const now = Math.floor(Date.now() / 1000);
    if (existing.length > 0 && existing[0].values.length > 0) {
        // Update
        db.run(`UPDATE credentials SET
        access_token = ?,
        refresh_token = ?,
        expires_at = ?,
        scope = ?,
      ...
      WHERE account_id = ?`, [
        ...
        // Insert
        db.run(`INSERT INTO credentials
...
export function getCredential(accountId) {
    const db = getDatabase();
    const result = db.exec(`SELECT * FROM credentials WHERE account_id = ?`, [accountId]);
...
export function getActiveCredential() {
    const db = getDatabase();
    const result = db.exec(`SELECT * FROM credentials ORDER BY updated_at DESC LIMIT 1`);
    if (result.length === 0 || result[0].values.length === 0) {
        return null;
    }
    const columns = result[0].columns;
...
export function listCredentials() {
    const db = getDatabase();
    const result = db.exec(`SELECT * FROM credentials ORDER BY updated_at DESC`);
    if (result.length === 0) {
        return [];
    }
    const columns = result[0].columns;
