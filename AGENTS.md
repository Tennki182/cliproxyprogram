...
- **额外检查**: `noImplicitReturns` 和 `noFallthroughCasesInSwitch` 已启用
- 使用显式类型；尽可能避免 `any`
...
1. **显式前缀**: `provider/model` 格式直接路由到指定提供者（如 `openrouter/gpt-4`）
2. **OpenAI 兼容提供者**: 检查前缀匹配和模型支持
3. **内置提供者**: Codex、iFlow、Gemini 按配置顺序检查
...
- **最大队列大小**: 100
- **并发数**: 可配置（默认 5，通过 `queue.concurrency`）
- **指数退避**: 基础 200ms，倍数 1.5，最大 2000ms
- **最大重试次数**: 可配置（默认 10）
- **重试策略**: 所有错误都允许重试
...
```yaml
# 控制是否将思维链返回到前端
# 启用后，思维链在 reasoning_content 字段中返回
# 默认值: true
...
```yaml
payload:
  default:        # OpenAI 格式默认参数
    - models: [{ name: "gemini-*" }]
      params: { thinkingBudget: 8192 }
  defaultRaw:     # Gemini 格式默认参数
  override:       # 强制覆盖参数
  overrideRaw:    # Gemini 格式强制覆盖
  filter:         # 过滤掉指定参数
    - models: [{ name: "*" }]
      params: ["user"]
```
...
所有管理路由需要 `Bearer <secret>` 认证（使用 `management.secret` 或回退到 `auth.loginSecret`）。
...
访问 `/admin` 或根路径可进入 Web 管理面板，功能包括：

- **概览**: 系统状态、运行时间、账户数量
- **认证**: 一键 OAuth 登录各提供者，支持远程认证
- **聊天**: 多协议测试（OpenAI/Anthropic/Gemini）
- **模型**: 查看所有可用模型，按提供者筛选
- **Provider**: 动态添加 OpenAI 兼容提供者
- **管理**: 凭证管理、配置热重载、API Key 设置
- **统计**: 请求统计、Token 使用量
- **日志**: 实时请求日志流
...
对于远程部署，提供专门的远程认证端点：
- `/auth/remote/gemini` - Gemini 远程认证
- `/auth/remote/codex` - Codex 远程认证
- `/auth/remote/iflow` - iFlow 远程认证

流程：在新标签页完成 OAuth → 复制回调 URL → 粘贴到远程认证窗口完成认证。
